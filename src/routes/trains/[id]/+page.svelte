<script lang="ts">
	...
	const hasSoundDecoders = $derived(
		data.confirmedDecoders.some(d => d.soundDecoder) ||
		data.formatDecoders.some(d => d.soundDecoder)
	);
...
<div class="jr-card p-6 mb-6">
	<div class="flex items-start justify-between">
		<div>
			<div class="flex items-center gap-2 mb-2">
				<span class="text-xs font-bold px-2 py-0.5 rounded-sm"
					style="background: {data.train.scale === 'HO' ? 'var(--color-jrc-orange)' : 'var(--color-green)'}; color: #fff; letter-spacing: 0.08em;">
		...
		<a href="/suggest?trainId={data.train.id}"
	...
	{#if data.train.notes}
		<p class="mt-4 text-sm p-3 rounded"
...
{#if data.compatFormats.length === 0}
	<div class="text-center py-12" style="color: var(--color-dim);">
		...
		<a href="/suggest?trainId={data.train.id}"
			class="inline-block mt-2 text-sm hover:underline" style="color: var(--color-green);">
...
{:else}
		...
		<div class="flex items-center gap-1 mb-6">
			{#each [['all', 'All'], ['basic', 'Basic'], ['sound', 'Sound']] as [val, label]}
				<button
					...
					{#if val === 'sound'}
						<svg class="inline w-3 h-3 mr-1 mb-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>
					{/if}
					{label}
				</button>
			{/each}
	...
	{#each data.compatFormats as fmt}
		...
		{#if confirmed.length > 0 || formatOnly.length > 0 || decoderFilter === 'all'}
			<div class="mb-8">
				<!-- Format header -->
				<div class="flex items-center gap-3 px-4 py-2.5 mb-4 rounded"
					...
					{#if fmt.purpose}
						<span class="inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-sm"
				...
				{#if confirmed.length > 0}
					<p class="text-xs font-semibold tracking-widest uppercase mb-2 px-1 flex items-center gap-2" style="color: var(--color-green);">
...
{#snippet decoderCard(dec: any, isConfirmed: boolean)}
	<div class="jr-card-flat p-4 hover:border-[var(--color-green-mid)] transition-colors"
		style="{dec.soundDecoder ? 'border-top: 3px solid #7c3aed;' : ''}{isConfirmed ? 'border-left: 3px solid var(--color-green);' : ''}">
		...
		<div class="flex items-center justify-between mb-2">
			<div class="flex items-center gap-1" style="color: var(--color-green);">
				...
				{/if}
				{#if dec.soundDecoder}
					<svg class="w-3.5 h-3.5" style="color: #7c3aed;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" title="Sound"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>
				{/if}
			</div>
			{#if isConfirmed}
				<span class="text-xs font-semibold px-1.5 py-0.5 rounded-sm"
		...
		{/if}
		{#if dec.buyUrl}
			<a href={dec.buyUrl} target="_blank" rel="noopener noreferrer"
				class="inline-block mt-2 text-xs hover:underline" style="color: var(--color-green);">Buy →</a>
		{/if}
	</div>
{/snippet}
