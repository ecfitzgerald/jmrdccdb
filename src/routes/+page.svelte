<script lang="ts">
	...
	type SortCol = 'manufacturer' | 'scale' | 'name' | 'modelNumber' | 'operatorName' | 'line';
	...
	const filtered = $derived(() => {
		...
		return data.trains.filter((t) => {
			...
			if (
				qLower &&
				![t.name, t.manufacturer, t.modelNumber, t.operatorName ?? '', t.line ?? ''].some((s) => s.toLowerCase().includes(qLower))
			)
				return false;
			return true;
		});
...
<div class="jr-card-flat p-4 mb-2 flex flex-wrap gap-3 items-end">
	<div class="flex-1 min-w-48">
		...
		<input
			id="q"
			type="search"
			placeholder="Train name, manufacturer, model number, road name…"
			bind:value={q}
				...
				<tbody>
					{#each sorted as train, i (train.id)}
						...
						>
							...
							<td class="px-4 py-3 text-sm" style="color: var(--color-muted);">{train.line ?? '—'}</td>
										...
										>
											...
											<span class="inline-flex items-center gap-0.5 ml-1 opacity-70">
												...
												{#if fmt.soundDecoderCount > 0}
													<SoundIcon class="w-3 h-3" title="Sound decoders available" />
												{/if}
											</span>
