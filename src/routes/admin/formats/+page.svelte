	...
	<form method="POST" action="?/add" class="jr-card-flat border border-[var(--color-border)] rounded  p-5 mb-6 space-y-4">
		...
		<div class="grid grid-cols-3 gap-4">
			<div class="col-span-2">
				...
				<input id="fname" name="name" type="text" placeholder="Next18"
			...
			<div>
				...
				<input id="pinCount" name="pinCount" type="number" placeholder="18"
					class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-400" />
			</div>
		</div>
		<div>
			<label class="block text-xs font-medium text-[var(--color-muted)] mb-1" for="fdesc">Description</label>
			<input id="fdesc" name="description" type="text" placeholder="Brief description of the format"
				class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-400" />
		</div>
		<div class="w-32">
					...
					<td class="px-4 py-2 text-right">
						{#if fmt.trainCount === 0}
							<form method="POST" action="?/delete"
								onsubmit={(e) => { if (!confirm('Delete this format?')) e.preventDefault(); }}>
