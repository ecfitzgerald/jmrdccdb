...
<div class="grid grid-cols-3 gap-4 mb-8">
	...
	] as [count, label, href, cls]}
		<a {href} class="block jr-card-flat border border-[var(--color-border)] rounded p-5  hover:border-slate-300 transition-colors">
			...
			{#if count > 0 && label === 'Pending Suggestions'}
				<div class="mt-2 text-xs font-medium text-[var(--color-warn)] bg-[var(--color-warn-bg)] rounded px-2 py-0.5 inline-block">
					Needs review
				</div>
			{/if}
		</a>
