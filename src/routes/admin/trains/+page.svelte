...
{#if form?.error}
	<div class="bg-[var(--color-danger-bg)] border border-[var(--color-danger)] text-[var(--color-danger)] text-sm rounded p-3 mb-4">{form.error}</div>
{/if}
{#if form?.success}
	<div class="bg-[var(--color-ok-bg)] border border-[var(--color-green-mid)] text-[var(--color-green)] text-sm rounded p-3 mb-4">Saved successfully.</div>
{/if}

{#if showAdd}
	<form method="POST" action="?/add" class="jr-card-flat border border-[var(--color-border)] rounded  p-5 mb-6 space-y-4">
		...
		<div class="grid grid-cols-2 gap-4">
			<div>
				...
				<input name="manufacturer" type="text" placeholder="Kato"
		...
		<div>
			...
			<input name="name" type="text" placeholder="E235 Series Yamanote Line (11-car set)"
				class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-400" />
		</div>
		<div class="grid grid-cols-2 gap-4">
			<div>
				<label class="block text-xs font-medium text-[var(--color-muted)] mb-1">Model Number *</label>
				<input name="modelNumber" type="text" placeholder="10-1785"
					class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-400" />
			</div>
			<div>
				<label class="block text-xs font-medium text-[var(--color-muted)] mb-1">Road / Operator</label>
				<input name="roadName" type="text" placeholder="JR East"
					class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-400" />
			</div>
		</div>
		<div class="grid grid-cols-2 gap-4">
			<div>
				...
				<input name="era" type="text" placeholder="2015–present"
					class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-slate-400" />
			</div>
		</div>
		<div>
			...
			<div class="flex flex-col gap-3">
				{#each data.formats as fmt}
					<div class="flex items-center gap-3 p-2 rounded" style="border: 1px solid var(--color-border);">
						<input type="checkbox" name="formatIds" value={fmt.id} id="fmt-{fmt.id}" class="accent-slate-700 shrink-0" />
			...
			{#each data.trains as t (t.id)}
				<tr class="hover:bg-[var(--color-raised)]">
					...
					<td class="px-4 py-2 text-right">
						<form method="POST" action="?/delete" onsubmit={(e) => { if (!confirm('Delete this train?')) e.preventDefault(); }}>
